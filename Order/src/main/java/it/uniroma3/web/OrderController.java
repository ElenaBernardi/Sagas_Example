...
public class OrderController {
    ...
    public CreateOrderResponse newOrder(@RequestBody CreateOrderRequest request) {
        ...
        Order order = orderService.create(request.getConsumerId(), orderLineItems);
    ...
    private CreateOrderResponse makeCreateOrderResponse(Order order) {
        return new CreateOrderResponse(order.getId(), order.getConsumerId(),order.getOrderState().toString());
    }
    ...
    private GetOrderResponse makeGetOrderResponse(Order order) {
        List<LineItem> lineItems =
                order.getOrderLineItems()
                        ...
                        .collect(Collectors.toList());
        return new GetOrderResponse(order.getId(), order.getConsumerId(), lineItems, order.getOrderState().toString());
